...
> **注**：阶段二十八实现的 `delta_dir`（同测站同测回所有方向共同扰动）在阶段二十九被回退，原因见阶段二十九说明。

---

### 阶段二十九：后视盘左读数精确为度盘零位（回退 delta_dir） ✅ 已完成

**目标：** 修正阶段二十八引入的 `delta_dir` 导致后视盘左读数不再精确为 0°/90° 的问题，恢复“后视读数为人为置盘的精确参考基准”这一实际观测逻辑。

**前置条件：** 阶段二十八已完成；已确认 `delta_dir` 同加于后视/前视虽然能保持水平角不变，但与实际方向观测法操作流程相矛盾。

**问题根因：**

实际方向观测法的操作顺序为：
1. 望远镜瞄准后视目标；
2. 观测员人为置盘，将水平度盘读数设定为预定值（如 0°、90°、180°、270°，或加上度盘零位偏移）；
3. 转动望远镜瞄准前视目标并读数。

因此，**后视读数是主动设定的参考基准，不应带有随机扰动**。

阶段二十八为了实现 docstring 中描述的“度盘零位不确定性”，将 `delta_dir` 同加于同一测站同一测回的所有方向（含后视）：
```python
L_back = L0 + delta_dir
L_fore = L0 + delta_dir + lbar + delta_set + delta_2c / 2.0
```

这导致输出中后视读数变为 `0°02'30.1"`、`90°02'29.5"` 等，不再精确等于度盘零位，与教学演示中“人为置盘”的概念冲突。

**修复方案（方案 B）：**

直接回退到阶段二十八之前的逻辑，移除 `delta_dir` 的使用：
```python
if target == back_name:
    L = L0                          # 后视盘左精确为 L0
    R = L0 - delta_2c + math.pi
else:
    L = L0 + lbar + delta_set + delta_2c / 2.0
    R = L0 + lbar + delta_set - delta_2c / 2.0 + math.pi
```

- 后视盘左读数精确等于 `L0`（0°、90°、... + 度盘偏移）；
- 水平角 = 前视方向值 - 后视方向值 = `lbar + delta_set`，仍保留测回间自然分散；
- `delta_2c` 盘左/盘右对称分配，2C 计算保持一致；
- 测回间角值差异仍由 `delta_set` 提供，教学展示效果不受影响。

**交付物:**
- `src/generators/traversing_generator.py`：
  - `_gen_station_angle()` 回退 `delta_dir`，后视盘左精确为 `L0`；
  - 更新 docstring，明确记录本阶段设计决策及回退原因；
  - `GenerationMetadata.angle_sigma_arcsec` 置为 `None`（因已无方向扰动）。
- `config/config_observation_program.json`：
  - `degree_plate_config.offset_arcsec` 从 `150` 调整为 `0`；
  - 同步更新 `offset_radians`、`offset_dms` 及 `examples`；
  - 目的：教学演示中后视读数显示为精确的 0°、90°、...，而非 0°02'30"、90°02'30"。
- `tests/test_traversing_generator.py`：新增 `TestBacksightLeftReadingExact`（3 项测试），多种子/多测回验证后视读数精确等于 `L0`，防止未来再次回归。
- `tests/test_config_loader.py`：调整默认配置测试，度盘偏移期望值改为 `0`。
- `AGENTS.md`：新增本阶段记录，明确说明回退原因及配置调整，防止后续阶段再次引入同加后视的 `delta_dir` 设计或恢复非零度盘偏移。

**关键技术点:**
- 后视读数是参考基准，必须精确；
- 所有“方向共同扰动”若同加于后视，都会破坏置盘概念；
- 测回间分散由 `delta_set` 独立承担，无需 `delta_dir`；
- `GenerationMetadata.angle_sigma_arcsec` 置 `None`，避免输出声明中出现已不存在的“导线角度扰动 σ”。

**验收标准:**
- 后视盘左读数在多种子、多测回下精确等于 `L0`（容差 < 1e-12 rad）；
- 前视读数仍自然分散，不等于 `L0`；
- 测回间角值仍不全相同；
- 全量测试通过。
